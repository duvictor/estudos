...
def CustomNet(width=128, height=128, depth=128):
    ...
    conv4 = layers.Conv3D(128, 3, padding='same', activation='relu', kernel_initializer='he_uniform')(pool3)
    pool4 = layers.MaxPool3D(padding="valid", data_format='channels_last')(conv4)


    # up sample
    dconv0 = layers.Conv3DTranspose(128, 3, padding='same', activation='relu', kernel_initializer='he_uniform')(pool4)
    up0 = layers.UpSampling3D(2)(dconv0)

    dconv1 = layers.Conv3DTranspose(64, 3, padding='same', activation='relu', kernel_initializer='he_uniform')(up0)
...
display_list_local = list()
display_list_local.append(sample_image)
display_list_local.append(sample_mask)

for i in range(len(display_list_local)):
    plt.subplot(1, len(display_list_local), i + 1)
    plt.title(title[i])
    # plotando o slice 64 do exame
    plt.imshow(display_list_local[i][64])
    plt.axis('off')
plt.show()
...
def show_predictions(dataset=None, num=1):
    if dataset:
        ...
        predito = model.predict(sample_image[tf.newaxis, ...])[0,:,:,:,0]
        display([sample_image, sample_mask,predito])
